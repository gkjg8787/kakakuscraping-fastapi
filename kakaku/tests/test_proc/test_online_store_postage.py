...
def test_needs_update_by_campaign_msg_no_campaign(test_db):
    ...
    ret = osp.needs_update_by_campaign_msg(db_list=db_dict_list, prefinfo=PrefInfoMock())
    assert ret == True

def test_needs_update_by_campaign_msg_range_campaign(test_db):
    db_dict_list = [
        {"pref_id":1, "campaign_msg":"2,000円以上お買い上げで送料無料キャンペーン 2024/01/03 00:00 ～ 2024/01/31 23:59 2,000円未満 500～1,500円 2,000円以上 送料無料"},
        {"pref_id":1, "campaign_msg":""},
    ]
    ret = osp.needs_update_by_campaign_msg(db_list=db_dict_list, prefinfo=PrefInfoMock())
    assert ret == True
...
def test_needs_update_by_campaign_msg_fix_campaign(test_db):
    db_dict_list = [
        {"pref_id":1, "campaign_msg":""},
        {"pref_id":1, "campaign_msg":"送料キャンペーン 2024/01/01 00:00 ～ 2024/01/31 23:59 100円未満 700円 10,000円未満 400円 10,000円以上 送料無料"},
    ]
    ret = osp.needs_update_by_campaign_msg(db_list=db_dict_list, prefinfo=PrefInfoMock())
    assert ret == False
...
def test_needs_update_by_campaign_msg_fix_campaign_not_common(test_db):
    db_dict_list = [
        {"pref_id":2, "campaign_msg":"送料キャンペーン 2024/01/01 00:00 ～ 2024/01/31 23:59 100円未満 700円 10,000円未満 400円 10,000円以上 送料無料"},
        {"pref_id":1, "campaign_msg":""},
    ]
    ret = osp.needs_update_by_campaign_msg(db_list=db_dict_list, prefinfo=PrefInfoMock())
    assert ret == True
