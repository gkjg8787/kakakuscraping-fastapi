...
class SearchOptionSettings:
    ...
    def create_category(self, value: dict):
        return SearchOptionSetting.create(
            value, filter_name.FilterQueryName.CATEGORY.value
        )

    def create_zaiko(self, value: dict):
        return SearchOptionSetting.create(
            value, filter_name.FilterQueryName.ZAIKO.value
        )

    def create_safesearch(self, value: dict):
        return SearchOptionSetting.create(
            value, filter_name.FilterQueryName.SAFES.value
        )

    def create_listview(self, value: dict):
        return SearchOptionSetting.create(
            value, filter_name.FilterQueryName.LISTVIEW.value
        )
...
class SearchExternalSiteContext(BaseTemplateValue):
    ...
    def __init__(self, request, sfq: SearchFilterQuery):
        ...
        self.categorylist = self.get_category_list(
            optjson=optjson, fq=fq, soss=soss, isdef=is_default_setting
        )
        self.stores = self.get_store_list(
            optjson=optjson, fq=fq, soss=soss, isdef=is_default_setting
        )
    ...
    def set_safes(
        self, sfq: SearchFilterQuery, soss: SearchOptionSettings, isdef: bool
    ):
        ...
        if (
            filter_name.FilterQueryName.SAFES in sfq.setting_list
    ...
    def set_zaiko(
        ...
        if (
            filter_name.FilterQueryName.ZAIKO in sfq.setting_list
    ...
    def set_listview(
        ...
        if (
            filter_name.FilterQueryName.LISTVIEW in sfq.setting_list
    ...
    def get_category_list(
        ...
        if filter_name.FilterQueryName.CATEGORY.value in fq:
            ...
            selected_id = self.get_category_selected_of_setting(
                category=category, soss=soss
            )
    ...
    def get_store_list(
        ...
        if filter_name.FilterQueryName.STORE.value in fq:
            ...
            selected_id_list = self.get_store_select_of_setting(
                stores=stores, soss=soss
            )
    ...
    def get_store_select_of_setting(self, stores: dict, soss: SearchOptionSettings):
        opt: StoreSelectOption | None = soss.get_option(
            filter_name.FilterQueryName.STORE
        )
        if not opt:
            return None
        results: list[int] = []
...
class SearchToAddContext(BaseTemplateValue):
    ...
    def get_item_list(self, db: Session, add_item_id: int):
        ...
        for item in newest_list:
            ...
            additem = AddSelectItem(item_id=d["item_id"], name=d["name"])
            if add_item_id != const_value.NONE_ID and add_item_id == additem.item_id:
