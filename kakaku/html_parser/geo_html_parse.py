...
class GeoGuideParse:
    ...
    def parse_guide(self, soup: BeautifulSoup):
        ...
        for p in p_list:
            target_text = re.sub(r"\s+", "", p.text.replace(",", ""))
            ...
            if not fs_m:
                continue
            if not fs_m[0][0] or not fs_m[0][1] or not fs_m[0][2]:
                continue
        ...
        if not boundary_val:
            return [
                self.create_parsestorepostage(
                    boundary_ope="<=", boundary_val=0, postage=basic_postage
                )
            ]
