...
def test_read_users_groups_edit_update_no_data(test_db):
    ...
    check_redirect(
        response,
        [
            RedirectCheckValue(status_code=302, location="/groups/edit/"),
            REDIRECT_READ_USERS,
            REDIRECT_READ_USERS,
        ],
    )
...
def test_read_users_groups_edit_update_update_item(test_db):
    ...
    check_redirect(
        response, [RedirectCheckValue(status_code=302, location="/groups/edit/")]
    )
    assert response.status_code == 200
    drop_test_db()
...
def test_read_users_groups_rename_no_data(test_db):
    ...
    check_redirect(
        response, [REDIRECT_READ_USERS]
    )  # [ REDIRECT_READ_USERS, REDIRECT_READ_USERS ])
    assert response.status_code == 200
    is_html(response.text)
...
def test_read_users_analysis_data_update(test_db, mocker):
    m = mocker.patch(
        "proc.system_status.SystemStatusAccess.getStatus",
        return_value=system_status.SystemStatus.DATA_UPDATE,
    )
    response = client.get(f"{prefix}/items/analysis/")
    assert response.status_code == 200
    is_html(response.text)
...
def test_read_users_online_stores_copy_overwrite(test_db):
    ...
    params = {
        filter_name.FilterQueryName.PREF.value: "東京都",
