...
from accessor.item import ItemQuery, UrlQuery
...
def send_item_list(item_id_list :List[int]):
    item_id_url_list = [dict(ret._mapping.items()) for ret in UrlQuery.get_act_items_url()]
...
def param_parser(argv):
    ...
    group.add_argument('-u', '--url',metavar='urlpath',help='update url list', type=str, nargs='+')
    group.add_argument('-i', '--itemid',metavar='item_id',help='update item_id list', type=int, nargs='+')
    group.add_argument('--urlall', help='update act all url', action='store_true')
    group.add_argument('-do','--db_organize', help='db organize', choices=['days', 'sync'])    
