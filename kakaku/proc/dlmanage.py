...
class DlProc:
    ...
    def putDlTask(self, url, itemid):
        ...
        if (parsed_url.netloc not in self.dlproclist):
            ...
            logger.info(get_filename() + ' create proc id=' + str(self.dlproclist[parsed_url.netloc].id) \
    ...
    def shutdownAll(self):
        ...
        if (len(self.dlproclist)>0):
            for v in self.dlproclist.values():
                v.proc.terminate()
                logger.info(get_filename() + ' dlproc terminate id=' + str(v.id) + ' host='+v.hostname)
