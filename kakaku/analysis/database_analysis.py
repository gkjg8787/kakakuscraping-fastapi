...
def not_exist_datetime_in_dict(dic: dict):
    if AnalysisKeyName.START not in dic or AnalysisKeyName.END not in dic:
        return True
    if AnalysisKeyName.START in dic and not dic[AnalysisKeyName.START]:
        return True
    if AnalysisKeyName.END in dic and not dic[AnalysisKeyName.END]:
        return True
    return False
...
class MultiPriceTrend(BaseModel):
    ...
    def __init__(
        ...
        if ct.is_down and pt.is_down:
            ...
            if max_rate == up_rate or max_rate == start_rate:
                trend = "上昇"
                is_up = True
                value = 1
            elif max_rate == down_rate or max_rate == end_rate:
                trend = "下落"
                is_down = True
                value = -1
...
class ItemPriceResult(BaseModel):
    ...
    def __init__(
        ...
        to_in_stock = PriceResult(
            all_count=all_count, count=to_in_stock_count, average=0
        )
        to_out_of_stock = PriceResult(
            all_count=all_count, count=to_out_of_stock_count, average=0
        )
        no_stock = PriceResult(all_count=all_count, count=no_stock_count, average=0)

        super().__init__(
            updown_trend=MultiPriceTrend(
                up_count, down_count, up_average, down_average
            ),
            up=up,
            down=down,
            no_change=no_change,
...
class UrlDownLoadResult(BaseModel):
    ...
    def __init__(
        ...
        super().__init__(
            trend=CountTrend(start_suc_rate, end_suc_rate),
...
class UrlStoreCountAverageResult(BaseModel):
    ...
    def __init__(self, start_dict: dict, end_dict: dict):
        ...
        super().__init__(
            trend=CountTrend(start_ave, end_ave),
            start=start_ave,
            end=end_ave,
        )

    def create_average(self, store_per_url: dict):
        all_store_count_list = [len(spu) for spu in store_per_url.values()]
...
class LogAnalysisResult:
    ...
    def get_start_itemid_act(self) -> set:
        return self.result[AnalysisKeyName.PERIOD_ITEMID][AnalysisKeyName.START][
            AnalysisKeyName.ACT
        ]

    def get_start_itemid_inact(self) -> set:
        return self.result[AnalysisKeyName.PERIOD_ITEMID][AnalysisKeyName.START][
            AnalysisKeyName.INACT
        ]

    def get_end_itemid_act(self) -> set:
        return self.result[AnalysisKeyName.PERIOD_ITEMID][AnalysisKeyName.END][
            AnalysisKeyName.ACT
        ]
    ...
    def get_common_itemid_act(self) -> set:
        return self.result[AnalysisKeyName.PERIOD_ITEMID][AnalysisKeyName.COMMON][
            AnalysisKeyName.ACT
        ]

    def get_start_urlid_act(self) -> set:
        return self.result[AnalysisKeyName.PERIOD_URLID][AnalysisKeyName.START][
            AnalysisKeyName.ACT
        ]
    ...
    def get_end_urlid_act(self) -> set:
        return self.result[AnalysisKeyName.PERIOD_URLID][AnalysisKeyName.END][
            AnalysisKeyName.ACT
        ]
    ...
    def get_common_urlid_act(self) -> set:
        return self.result[AnalysisKeyName.PERIOD_URLID][AnalysisKeyName.COMMON][
            AnalysisKeyName.ACT
        ]

    def get_item_price(self) -> ItemPriceResult:
        return ItemPriceResult(
            up_dict=self.get_itemprice_up_dict(),
    ...
    def get_start_itemprice_average(self) -> int:
        return int(
            self.result[AnalysisKeyName.ITEM_PRICE][AnalysisKeyName.START][
                AnalysisKeyName.AVERAGE
            ]
        )

    def get_end_itemprice_average(self) -> int:
        return int(
            self.result[AnalysisKeyName.ITEM_PRICE][AnalysisKeyName.END][
                AnalysisKeyName.AVERAGE
            ]
        )
    ...
    def get_itemprice_up_dict(self) -> dict:
        return self.result[AnalysisKeyName.ITEM_PRICE][AnalysisKeyName.PRICE][
            AnalysisKeyName.UP
        ]

    def get_itemprice_down_dict(self) -> dict:
        return self.result[AnalysisKeyName.ITEM_PRICE][AnalysisKeyName.PRICE][
            AnalysisKeyName.DOWN
        ]

    def get_itemprice_no_change_list(self) -> list:
        return list(
            self.result[AnalysisKeyName.ITEM_PRICE][AnalysisKeyName.PRICE][
                AnalysisKeyName.NO_CHANGE
            ].keys()
        )

    def get_itemprice_to_in_stock_list(self) -> list:
        return self.result[AnalysisKeyName.ITEM_PRICE][AnalysisKeyName.PRICE][
            AnalysisKeyName.TO_IN_STOCK
        ]

    def get_itemprice_to_out_of_stock_list(self) -> list:
        return self.result[AnalysisKeyName.ITEM_PRICE][AnalysisKeyName.PRICE][
            AnalysisKeyName.TO_OUT_OF_STOCK
        ]

    def get_itemprice_no_stock_list(self) -> list:
        return self.result[AnalysisKeyName.ITEM_PRICE][AnalysisKeyName.PRICE][
            AnalysisKeyName.NO_STOCK
        ]

    def get_url_download(self) -> UrlDownLoadResult:
        return UrlDownLoadResult(
            start_success=len(self.get_start_url_success_list()),
    ...
    def get_start_url_success_list(self) -> list:
        return list(
            self.result[AnalysisKeyName.URL][AnalysisKeyName.START][
                AnalysisKeyName.SUCCESS
            ].keys()
        )
    ...
    def get_start_url_failed_list(self) -> list:
        return list(
            self.result[AnalysisKeyName.URL][AnalysisKeyName.START][
                AnalysisKeyName.FAILED
            ].keys()
        )

    def get_end_url_success_list(self) -> list:
        return list(
            self.result[AnalysisKeyName.URL][AnalysisKeyName.END][
                AnalysisKeyName.SUCCESS
            ].keys()
        )
    ...
    def get_store_count(self) -> CountResult:
        return CountResult(
            start=len(self.get_start_store_dict().keys()),
    ...
    def get_start_store_dict(self) -> dict:
        return self.result[AnalysisKeyName.URL][AnalysisKeyName.START][
            AnalysisKeyName.STORE
        ]

    def get_end_store_dict(self) -> dict:
        return self.result[AnalysisKeyName.URL][AnalysisKeyName.END][
            AnalysisKeyName.STORE
        ]
...
class LogAnalysis:
    ...
    def __init__(self, data):
        ...
        convertdata: dict = {}
        convertdata[AnalysisKeyName.PERIOD_DATETIME] = {
            AnalysisKeyName.START: start_datetime,
            AnalysisKeyName.END: end_datetime,
        }
        convertdata.update(
            self.get_period_end_itemid_and_urlid(
                dict_list=dict_list,
                start_datetime=convertdata[AnalysisKeyName.PERIOD_DATETIME][
                    AnalysisKeyName.START
                ],
                end_datetime=convertdata[AnalysisKeyName.PERIOD_DATETIME][
                    AnalysisKeyName.END
                ],
            )
        )
        ...
        item_price_analysis = self.get_item_price_analysis(
            common_ids=convertdata[AnalysisKeyName.PERIOD_ITEMID][
                AnalysisKeyName.COMMON
            ][AnalysisKeyName.ACT],
            start_data=period_end_log[AnalysisKeyName.PERIOD_LOG][
                AnalysisKeyName.START
            ],
            end_data=period_end_log[AnalysisKeyName.PERIOD_LOG][AnalysisKeyName.END],
        )
        url_analysis = self.get_url_analysis(
            common_ids=convertdata[AnalysisKeyName.PERIOD_URLID][
                AnalysisKeyName.COMMON
            ][AnalysisKeyName.ACT],
            start_data=period_end_log[AnalysisKeyName.PERIOD_LOG][
                AnalysisKeyName.START
    ...
    def get_url_analysis(
        ...
        result = {AnalysisKeyName.START: {}, AnalysisKeyName.END: {}}
        ...
        for id in common_ids:
            ...
            def analyse_url_log(log_list: list, key: str):
                for log in log_list:
                    if (
                        id not in result[key][AnalysisKeyName.SUCCESS]
                        and id not in result[key][AnalysisKeyName.FAILED]
                    ):
                        if is_init_price(log["usedprice"]) and is_init_price(
                            log["newprice"]
                        ):
    ...
    def get_item_price_analysis(
        ...
        for id in common_ids:
            ...
            if is_init_price(end_item_prices[id]):
                ...
                init_price_exist = True
            if init_price_exist:
                if is_init_price(start_item_prices[id]) and not is_init_price(
                    end_item_prices[id]
                ):
                    result[AnalysisKeyName.PRICE][AnalysisKeyName.TO_IN_STOCK].append(
                        id
                    )
                elif not is_init_price(start_item_prices[id]) and is_init_price(
                    end_item_prices[id]
                ):
                    result[AnalysisKeyName.PRICE][
                        AnalysisKeyName.TO_OUT_OF_STOCK
        ...
        result[AnalysisKeyName.START][AnalysisKeyName.AVERAGE] = statistics.mean(
            list(start_item_prices.values())
        )
        result[AnalysisKeyName.END][AnalysisKeyName.AVERAGE] = statistics.mean(
            list(end_item_prices.values())
        )
        result[AnalysisKeyName.START][AnalysisKeyName.MEDIAN] = statistics.median(
            list(start_item_prices.values())
        )
        result[AnalysisKeyName.END][AnalysisKeyName.MEDIAN] = statistics.median(
            list(end_item_prices.values())
        )
    ...
    @staticmethod
    def get_period_end_log(
        dict_list: list[dict], start_datetime: datetime, end_datetime: datetime
    ):
        start_date = utcTolocaltime(start_datetime).date()
        end_date = utcTolocaltime(end_datetime).date()

        result = {AnalysisKeyName.PERIOD_LOG: {}}
        for dic in dict_list:
            if start_date == LogAnalysis.datetime_to_date(dic["created_at"]):
                if AnalysisKeyName.START in result[AnalysisKeyName.PERIOD_LOG]:
                    result[AnalysisKeyName.PERIOD_LOG][AnalysisKeyName.START].append(
                        dic
                    )
                else:
    ...
    def get_period_end_datetime(dict_list: list[dict]):
        ...
        max = None
        for dic in dict_list:
            if not min or min > dic["created_at"]:
                min = dic["created_at"]
            if not max or max < dic["created_at"]:
                max = dic["created_at"]
        return min, max
    ...
    @staticmethod
    def get_period_end_itemid_and_urlid(
        dict_list: list[dict], start_datetime: datetime, end_datetime: datetime
    ):
        result = {}
        ...
        for dic in dict_list:
            if start_date == LogAnalysis.datetime_to_date(dic["created_at"]):
                ...
                if UrlActive.INACTIVE.value == dic["active"]:
                    itemid_dic.add_to_start(
                        suf=AnalysisKeyName.INACT, val=dic["item_id"]
                    )
