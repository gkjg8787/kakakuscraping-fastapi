...
def test_read_admin_dashboard_post_no_data():
    response = client.post(
        f'{prefix}/dashboard/post/',
    )
    assert response.status_code == 422



def test_read_admin_dashboard_post_startup_and_stop():
    response = client.post(
        f'{prefix}/dashboard/post/',
...
def test_read_admin_dashboard_post_startup_and_restart():
    response = client.post(
        f'{prefix}/dashboard/post/',
        data={filter_name.DashBoardPostName.SYSTEM_CTRL_BTN.value:filter_name.SystemCtrlBtnName.STARTUP.value},
    )
