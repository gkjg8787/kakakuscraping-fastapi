...
class ItemAutoUpdateTimerFactory:
    ...
    def create(db :Session, logger :Logger):
        ...
        if not upts\
            ...
            logger.info(f"{get_filename()} {__class__.__name__} no updatetime list = {upts}")
            isAuto = False
            upts = []
...
class ItemAutoUpdateTimer:
    ...
    def __init__(self, isAutoUpdate :bool,
        ...
        self.updatelocaltimestrs = TwoDigitHourFormat.convet_list_to_two_digit_hour_list(updatelocaltimestrs,
                                                                                         logger
                                                                                         )
        ...
        for uplt in sorted(self.updatelocaltime, reverse=True):
            if lt >= uplt:
                if not self.isUpdatefinished(db=db, start=uplt, end=next_time):
                    self.logger.info(get_filename() + " sendTask "+ ScrOrder.AUTO_UPDATE_ACT_ALL + ", target=" + uplt.strftime("%H:%M"))
    ...
    def run(self, db :Session, reset=True):
        ...
        if reset\
            ...
            self.logger.info(f"{get_filename()} {__class__.__name__} reset updatelocaltime")
    ...
    def removeUpdateTime(self, updated_time :Optional[datetime]):
        if not updated_time\
            ...
            return
        self.logger.info(f'{get_filename()}  {__class__.__name__} remove older than {updated_time} from updatelocaltime')
        results = [upt for upt in self.updatelocaltime if upt > updated_time]
        self.updatelocaltime = results
        self.logger.debug(f'{get_filename()}  {__class__.__name__} updatelocaltime = {self.updatelocaltime}')
    
    def isUpdatefinished(self, db :Session, start :datetime, end :datetime):
        ret = SystemStatusLogAccess.get_count_by_systemstatus_and_datetime_range(db,
                                                                                 syssts_name_list=[SystemStatusLogName.ALL_DATA_UPDATE,
...
class DailyOnlineStoreUpdate:
    ...
    def __init__(self, logger :Logger):
        ...
        self.logger = logger
        self.updatelocaltime = []
        self.updatelocaltimestrs = TwoDigitHourFormat.convet_list_to_two_digit_hour_list(upts,
                                                                                         logger
                                                                                         )
    ...
    def check_update_time(self, db :Session):
        ...
        for uplt in sorted(self.updatelocaltime, reverse=True):
            if lt >= uplt:
                if not self.isUpdatefinished(db=db, start=uplt, end=next_time):
                    self.logger.info(get_filename() + " sendTask "+ ScrOrder.UPDATE_ONLINE_STORE_POSTAGE)
    ...
    def isUpdatefinished(self, db :Session, start :datetime, end :datetime):
        update_count = SystemStatusLogAccess.get_count_by_systemstatus_and_datetime_range(db,
        ...
        if update_count > 1:
            return True
        not_update_store_count = ac_store.OnlineStoreQuery.get_count_postage_not_updated_today(db)
...
class TimerProc:
    ...
    def start(self):
        ...
        if self.iaut.isAutoUpdate:
            iutproc = Process(target=self.run_itemupdatetimer, args=(read_config.get_auto_update_check_cycle_time(),))
            self.proc_list.append(iutproc)
            logger.info(f"{get_filename()} start ItemAutoUpdateTimer")
            
        self.dlo = DailyLogOrganizer(logger)
        dloproc = Process(target=self.run_dailylogorganizer, args=(read_config.get_auto_db_organizer_check_cycle_time(),))
        ...
        if self.dosu.isAutoUpdate:
            osuproc = Process(target=self.run_onlinestoreupdate, args=(read_config.get_auto_update_online_store_cycle_time(),))
