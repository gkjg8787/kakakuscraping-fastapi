...
class ShippingTermsBoundary:
    ...
    def create_boundary_of_db(
        ...
        if lower_ope not in cls.boundary_operation_list:
            raise ValueError("unsupported boundary operation")
    ...
    @classmethod
    def add_terms_to_boundary(cls, boundary: str, add_ope: str, add_val: int):
        ...
        if lower_ope == upper_ope:
            if lower_ope == ">" or lower_ope == ">=":
                return cls.create_boundary_of_db(
                    lower_ope=lower_ope, lower_val=lower_val
                )
            if lower_ope == "<" or lower_ope == "<=":
                return cls.create_boundary_of_db(
                    lower_ope=upper_ope, lower_val=upper_val
                )
