...
class ExtractServerLogByOpenFile(IExtractServerLog):
    def get_list_by_time_period(
        self,
        logname: str,
        start: datetime | None = None,
        ...
        with open(fpath, "r") as f:
            serverlogfile = ServerLogFileFactory.create(f.read(), filename=filename)
        if not serverlogfile:
            return ExtractServerLogResult(
                filename=filename, error_msg="can't create log file object"
            )
        if not start and not end and not loglevel_name_list:
            return ExtractServerLogResult(
                filename=filename,
                text="\n".join(ServerLogRawText().toTextList(logfile=serverlogfile)),
            )

        results: list[ServerLogLine] = []
        all_loglevel: bool = self.is_all_loglevel(loglevel_name_list=loglevel_name_list)
        for l in serverlogfile.logs:
            if not all_loglevel and not self.in_loglevelname(
                loglevel_name_list=loglevel_name_list, serverlogline=l
            ):
                continue
            if not self.within_period(line=l, start=start, end=end):
                continue
            results.append(l)
        serverlogfile.logs = results
        return ExtractServerLogResult(
            filename=filename,
            text="\n".join(ServerLogRawText().toTextList(logfile=serverlogfile)),
        )

    def is_all_loglevel(self, loglevel_name_list: list[str]) -> bool:
        for loglevelname in loglevel_name_list:
            if filter_name.LogLevelFilterName.ALL.name.lower() == loglevelname.lower():
                return True
        return False

    def in_loglevelname(
        self, loglevel_name_list: list[str], serverlogline: ServerLogLine
    ) -> bool:
        if not loglevel_name_list:
            return True
        for loglevelname in loglevel_name_list:
            if loglevelname.lower() == serverlogline.loglevel.lower():
                return True
        return False

    def within_period(
        self, line: ServerLogLine, start: datetime | None, end: datetime | None
    ) -> bool:
        if not start and not end:
            return True
        if start and end:
            return (
                line.created_at.date() >= start.date()
                and line.created_at.date() <= end.date()
            )
        if start:
            return line.created_at.date() >= start.date()
        if end:
            return line.created_at.date() <= end.date()
