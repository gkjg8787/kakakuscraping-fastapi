...
class TestPredictionQuery:
    ...
    def test_get_dataframe_from_sql_by_url_id_start_only(self, test_temp_db):
        ...
        param = {
            "url_id": 1,
            "start": dbtimeTodatetime("2024-01-01 12:00:00").replace(
                tzinfo=timezone.utc
            ),
            "end": None,
        }
        ...
        df = self.get_dataframe_from_sql(
            stmt=PredictionQuery.get_stmt_pricelog_by_url_id_and_datetime_range(**param)
        )
        assert len(df) == 0
    ...
    def test_get_dataframe_from_sql_by_url_id_start_end(self, test_temp_db):
        ...
        assert len(df) == 3
        assert df["usedprice"].iloc[0] == 1250
        assert df["usedprice"].iloc[2] == 1600
    ...
    def test_get_dataframe_from_sql_by_item_id_concat_url_log_start_only(
        ...
        param = {
            "item_id": 1,
            "start": dbtimeTodatetime("2024-01-01 10:01:00").replace(
                tzinfo=timezone.utc
            ),
            "end": None,
        }
        ...
        df = self.get_dataframe_from_sql(
            stmt=PredictionQuery.get_stmt_pricelog_by_item_id_and_datetime_range(
                **param
            )
        )
        assert len(df) == 1
        assert df["usedprice"].iloc[0] == 1200
