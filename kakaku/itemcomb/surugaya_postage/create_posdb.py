...
from itemcomb.surugaya_postage.const_value import SHOPLIST_URL
...
from accessor.store import DailyOnlineShopInfoQuery, DailyOnlineShopInfo
...
def is_todays_data_dailyonlineshopinfo(db: Session):
    cnt = DailyOnlineShopInfoQuery.get_todays_count(
        db=db, insert_proc_type_list=[InsertProcType.MAKEPURE_TOOL.value]
    )
    if cnt and cnt > 0:
        return True
    return False
...
def is_expired_dailyonlineshopinfo(db: Session):
    cnt = DailyOnlineShopInfoQuery.get_count_before_today(
        db=db, insert_proc_type_list=[InsertProcType.MAKEPURE_TOOL.value]
    )
    if cnt and cnt > 0:
        return True
...
def create_dailyonlineshopinfo(db: Session):
    DailyOnlineShopInfoQuery.delete(
        db=db, insert_proc_type_list=[InsertProcType.MAKEPURE_TOOL.value]
    )
    sm = SurugayaMakepure()
