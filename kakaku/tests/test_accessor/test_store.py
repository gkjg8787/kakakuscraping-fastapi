...
def test_daily_online_shop_info_delete_insert_proc_type_1(test_db):
    ...
    assert cnt == 3
    store.DailyOnlineShopInfoQuery.delete(
        db=test_db, insert_proc_type_list=[posd.InsertProcType.ITEM_UPDATE.value]
    )
...
def test_daily_online_shop_info_get_count_before_today(test_db):
    ...
    for a in ret:
        ...
        up_list.append(a)
        break
    store.DailyOnlineShopInfoQuery.update(db=test_db, update_list=up_list)
...
def test_daily_online_shop_info_get_by_contains_storename(test_db):
    db_test_data.add_daily_online_shop_data_set_1(test_db)
    storename = "千葉"
    ret = store.DailyOnlineShopInfoQuery.get_by_contains_storename(
        test_db, storename=storename
    )
    assert len(ret) == 1
