...
def test_read_search_top_option_hide_safes(mocker):
    ...
    mocker.patch('common.read_config.get_search_options_default', return_value=mock_config )
    response = client.get(
        f'{prefix}/',
    )
    assert response.status_code == 200
...
def test_read_search_top_option_checked_safes_off(mocker):
    ...
    assert response.status_code == 200
    assert '<input type="checkbox" name="safes" form="search_inputform" id="safe_search_checkbox" value="0" checked>' in response.text
...
def test_read_search_top_option_no_check_zaiko(mocker):
    ...
    assert response.status_code == 200
    assert '<input type="checkbox" name="zaiko" form="search_inputform" id="zaiko_checkbox" value="1" >' in response.text
...
def test_read_search_top_option_checked_zaiko(mocker):
    ...
    assert response.status_code == 200
    assert '<input type="checkbox" name="zaiko" form="search_inputform" id="zaiko_checkbox" value="1" checked>' in response.text
...
def test_read_search_top_option_no_check_listview(mocker):
    ...
    assert response.status_code == 200
    assert '<input type="checkbox" name="listview" form="search_inputform" id="listview_checkbox" value="1" >' in response.text
...
def test_read_search_top_option_checked_listview(mocker):
    ...
    assert response.status_code == 200
    assert '<input type="checkbox" name="listview" form="search_inputform" id="listview_checkbox" value="1" checked>' in response.text
