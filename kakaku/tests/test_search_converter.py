...
def test_convertToSearchResultPage_full():
    ...
    srp :SearchResultPage = SearchDictConverter.convertToSearchResultPage(pageinfo, urlparam)

    next_url = SearchDictConverter.createURLParam(urlparam, pageinfo[SearchCmn.CURRENT] + 1)
    pre_url = SearchDictConverter.createURLParam(urlparam, pageinfo[SearchCmn.CURRENT] - 1)
...
def test_convertToSearchResultPage_first_page():
    ...
    assert srp.next_url == SearchDictConverter.createURLParam(urlparam, pageinfo[SearchCmn.CURRENT] + 1)
...
def test_convertToSearchResultPage_max_page():
    ...
    assert srp.next_url == None
    assert srp.pre_url == SearchDictConverter.createURLParam(urlparam, pageinfo[SearchCmn.CURRENT] - 1)
    assert srp.more_page == False
    assert not srp.pages is None
    for page in srp.pages:
        if page.cure_page:
