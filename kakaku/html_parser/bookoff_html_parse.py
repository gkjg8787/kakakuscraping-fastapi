...
class BookoffOrderParse:
    ...
    def parse_order(self, soup :BeautifulSoup):
        ...
        p_list = soup.select_one("#order").find_next_sibling("div").select("div.UGContentTxt-indent p")
        for p in p_list:
            target_text = re.sub("\s+", "", p.text)
    ...
    def parse_free_shipping(self, target_text :str):
        m = re.findall(r"([1-9][0-9]+)円(以上)の注文で送料(無料)",
                       target_text.replace(",", "")
                       )
        if not m:
            return None
        if not m[0][0]\
            ...
            or not m[0][2]:
            return None
        psp = self.create_parsestorepostage(boundary_ope="<=", boundary_val=int(m[0][0]), postage=0)
        return psp
    
    def parse_country_wide_flat_rate(self,
                                    target_text :str,
                                    ):
        m = re.findall(r"([1-9][0-9]+)円(未満)の場合、日本全国一律([1-9][0-9]+)円",
                       target_text.replace(",", "")
                       )
        if not m:
            return None
        if not m[0][0]\
            ...
            or not m[0][2]:
            return None
        psp = self.create_parsestorepostage(boundary_ope=">", boundary_val=int(m[0][0]), postage=int(m[0][2]))
        return psp
